...
bool takeoff(ros::Publisher takeoff_pub, ros::Rate loop_rate);
bool increaseAltitude(ros::Publisher takeoff_pub, ros::Rate loop_rate);
bool land(ros::Publisher land_pub);
...
int main(int argc, char **argv) {
  ...
  // Begin main loop
  while (ros::ok()) {
  	if (batteryLevel < 20) { // Begin landing procedure drone doesn't crash and burn due to lacking battery
    ...
    if (!isTakenOff) {
      if (takeoff(takeoff_pub, loop_rate)) {
        isTakenOff = true;
        isRunning = true;
        isLanded = false;
      }
    ...
    //else if (isRunning){
      //if (increaseAltitude(fly_pub, loop_rate))
        //isRunning = false;
    //}
    else if (!isLanded) {
      if (land(land_pub)) {
...
bool increaseAltitude(ros::Publisher publisher, ros::Rate loop_rate) {
	if (actionStart == NULL) {
