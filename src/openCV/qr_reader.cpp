...
int processImage(Mat image)
{
  ...
  */
  ...
  Image qrpls(width, height, "Y800", raw, width * height);
   
  // scan the image for barcodes  
  int n = scanner.scan(qrpls);  
  
  // extract results  
  for(Image::SymbolIterator symbol = qrpls.symbol_begin();  
  symbol != qrpls.symbol_end();  
  ++symbol) {  
          vector<Point> vp;  
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  try
  {
    processImage(cv_bridge::toCvShare(msg, "rgb8")->image);
    cv::waitKey(30);
  }
